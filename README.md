...
- 🚫 **경유 자동 제외 + 직항 우선 정렬**: 직항이 있는 노선은 직항만 표시. 직항이 없거나 10시간 이상 장거리 노선만 경유 포함 (`LONGHAUL_MIN_HOURS` 로 조정). 경유가 함께 표시될 때도 직항이 먼저, 각 그룹 안에서 가격순
- 🧭 **다구간 검색**(`/multi`): 출발-도착-날짜를 구간 수만큼 반복 입력 (2~5개 구간, 가격은 전체 여정 총액)
- ⚙ **검색 옵션**: 명령 뒤에 `직항`, `2명`, `비즈니스`/`일등석` 을 붙이면 인식 (`/flight`·`/flex`·`/watch` 공통)
