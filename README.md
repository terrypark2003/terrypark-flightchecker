...
```
다음 주 금요일에 오사카 갔다 일요일에 오는 표    # 🤖 그냥 문장으로 (GEMINI_API_KEY 필요)
/flight 인천 후쿠오카 2026-06-06 2026-06-07   # 왕복 (한글 공항명 가능)
/flight ICN FUK 2026-06-06                   # 편도 (코드도 가능)
/flight 인천 방콕 2026-06-06 2명 비즈니스      # 옵션: 직항 / N명 / 비즈니스·일등석
/flex 인천 후쿠오카 2026-06-06 2026-06-07     # ±3일 날짜별 최저가 비교
/multi 인천 도쿄 2026-06-06 도쿄 오사카 2026-06-08 오사카 인천 2026-06-10  # 다구간
...
**기능 요약**
- 🤖 **자연어 검색** (선택): `다음 주 금요일에 오사카 갔다가 일요일에 오는 표` 처럼 그냥 문장을 보내면 Gemini가 조건을 해석해 검색·알림 등록까지 실행. `GEMINI_API_KEY` 환경변수 설정 시 활성화 ([aistudio.google.com/apikey](https://aistudio.google.com/apikey) 무료 발급, 해석만 Gemini가 하고 검색은 SerpApi라 SerpApi 한도에 영향 없음)
- 🛫 **한글 공항명 입력**: `인천`, `후쿠오카`, `도쿄` 등을 코드(ICN/FUK/NRT)로 자동 변환 (`flightchecker/airports.py`)
- 🚫 **직항 전용**: 모든 검색이 기본 직항만. `경유포함` 토큰을 붙일 때만 경유 표시 (그때도 직항 먼저, 각 그룹 안에서 가격순)
- 🧭 **다구간 검색**(`/multi`): 출발-도착-날짜를 구간 수만큼 반복 입력 (2~5개 구간, 가격은 전체 여정 총액)
- ⚙ **검색 옵션**: 명령 뒤에 `2명`, `비즈니스`/`일등석`, `경유포함` 을 붙이면 인식 (`/flight`·`/flex`·`/watch` 공통)
- 📅 **유연한 날짜 검색**(`/flex`): 기준일 ±3일의 날짜별 최저가를 비교해 가장 싼 날 표시
- 🔔 **가격 알림**(`/watch`): 목표가 이하로 떨어지면 자동 알림 (기본 6시간마다 검사, `WATCH_INTERVAL_SEC` 로 조정)
- 📉 **급락 감지**: 알림 등록한 노선이 최근 2주 평균보다 20% 이상 싸지면 목표가와 무관하게 자동 알림 (`DROP_ALERT_PCT` 로 조정)
- 📈 **가격 이력 그래프**(`/history`): 검색·알림 검사 때마다 최저가를 기록해 가격 변화를 그래프 이미지로 전송 (추가 API 호출 없음)
- 🛫🛬 **왕복 2단계 선택**: 왕복 검색 시 가는편 목록을 먼저 보여주고, 번호 버튼으로 가는편을 고르면 SerpApi `departure_token` 2차 조회로 **오는편 시간표 + 확정 왕복 총액**을 표시 (가는편 선택 시에만 검색 1회 추가 사용)
- 🛒 **예매 링크 버튼**: 검색 결과·가격 알림 메시지에 구글 항공권/스카이스캐너 바로가기 버튼
- 🎫 **스카이패스 마일리지 비교**: 한국 발착 노선 검색 시 대한항공 예상 공제 마일리지(지역대 공제표 기반)와 1마일당 가치를 계산해 "현금 vs 마일" 판단을 표시하고, 대한항공 보너스 좌석 조회 바로가기 버튼 제공. 공제표 변경 시 `flightchecker/mileage.py` 의 `_CHART` 만 수정하면 됨 (보너스 좌석 실시간 조회는 대한항공이 API를 제공하지 않아 미지원)
- 💬 **버튼 UI**(`/menu`): 출발지·도착지·날짜를 버튼으로 선택, 검색 결과에서 바로 "알림 -5%/-10%" 버튼으로 알림 등록
- 입력창 `/` 메뉴에 명령어 자동 등록, 날짜 형식 오류 시 안내
- 검색 로직은 CLI와 동일한 `flightchecker.search_flights()` 를 그대로 사용합니다.

> 가격 알림은 `watches.json`, 가격 이력은 `price_history.json` 에 저장됩니다 (git에는 커밋되지 않음).
> Railway 등에서 `WATCH_DB=/data/watches.json` 처럼 볼륨 경로를 지정하면 가격 이력도 같은 폴더에 저장됩니다.
